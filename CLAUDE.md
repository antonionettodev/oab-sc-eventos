...
14. **`development-standards.md`** – Development standards and conventions
    - Mandatory folder and file naming conventions (kebab-case, scope rules)
    - Module-based component organization
    - Next.js + Payload architectural patterns (Server Components, Server Actions)
    - Component file vs export naming rules
    - Client vs Server component guidelines
    - Hook naming and scoping conventions (frontend and backend)
    - Payload collection structure and modularization
    - Admin UI standards (labels, groups, placeholders, descriptions)
    - UI stack enforcement (Next.js App Router, TailwindCSS, shadcn/ui)
    - Design fidelity, responsiveness, and state handling
    - Reusability vs local code decision rules
    - Expected output structure when generating or adapting UI

15. **`payments-api.md`** – Payments and checkout API
    - Checkout creation endpoint (request flow and lifecycle)
    - Authentication and required headers
    - Request body structure and validation rules
    - Customer data handling and immutability rules
    - Items, pricing, discounts, and additional charges
    - Shipping configuration (fixed, free, calculated)
    - Accepted payment methods (Credit Card, Debit Card, Boleto, PIX)
    - Payment method configuration (installments and interest rules)
    - Redirect, return, and notification URLs
    - Checkout response object and status lifecycle
    - Recurring payments and subscription plans
    - Webhook configuration and event types
    - Webhook payload structure and examples
    - Refund and payment event handling

16. **`brconselhos-auth-api.md`** – BR Conselhos authentication web service
    - SOAP-based authentication service for validating legal professionals
    - Official Web Method: `Autenticar`
    - Service page: `WSAutenticar.asmx`
    - Production endpoint:
      - https://servicos.oab-sc.org.br/WSAutenticar/WSAutenticar.asmx?op=Autenticar
    - Authentication flow based exclusively on BR Conselhos user credentials
    - Login identifier rules:
      - Only **CPF or CNPJ** is accepted as the user identifier
      - OAB number or custom usernames are **not** used
    - No additional credentials required:
      ...
      - No extra headers
    - Request format:
      - SOAP XML over HTTP POST
      - Required parameters:
        - `Usuario` (CPF or CNPJ registered in BR Conselhos)
        - `Senha` (user password)
    - Response behavior:
      - Always returns an XML string
      - Authentication failure:
        - `<Status>Usuário e Senha inválidos</Status>`
      - Authentication success:
        - `<Status>OK</Status>` plus a `<Cadastro>` node
    - Returned data includes (when authentication is successful):
      - Registration data (council registration, temporary registration)
      - Personal data (name, birth date, parents)
      - Address data (CEP, street, city, state, country)
      - Contact data (commercial email, phone numbers)
      - Professional status (active/inactive, inadimplente, jovem advogado)
      - Identity data (CPF/CNPJ, RG, issuing authority, issue date)
      - Account metadata (login user, registration date, subsection)
    - The XML structure described in the official manual matches the current production response
    - Integration purpose:
      - Validate the professional’s credentials
      - Delegate authentication logic entirely to BR Conselhos
      - Consume returned data as authoritative professional records
